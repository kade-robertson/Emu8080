...
namespace Emu8080 
{
    ...
    public static class CPU 
    {
        ...
        private static Dictionary<byte, Action> Instructions = new Dictionary<byte, Action>() {
            ...
            { 0x40 /* MOV B, B   */, () => { /* Essentially a NOP. */ } },
            ...
            { 0x49 /* MOV C, C   */, () => { /* Essentially a NOP. */ } },
            ...
            { 0x52 /* MOV D, D   */, () => { /* Essentially a NOP. */ } },
            ...
            { 0x5B /* MOV E, E   */, () => { /* Essentially a NOP. */ } },
            ...
            { 0x64 /* MOV H, H   */, () => { /* Essentially a NOP. */ } },
            ...
            { 0x6D /* MOV L, L   */, () => { /* Essentially a NOP. */ } },
            ...
            { 0x70 /* MOV M, B   */, () => { Array.Copy(State.Registers, 0x01, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x71 /* MOV M, C   */, () => { Array.Copy(State.Registers, 0x02, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x72 /* MOV M, D   */, () => { Array.Copy(State.Registers, 0x03, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x73 /* MOV M, E   */, () => { Array.Copy(State.Registers, 0x04, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x74 /* MOV M, H   */, () => { Array.Copy(State.Registers, 0x05, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x75 /* MOV M, L   */, () => { Array.Copy(State.Registers, 0x06, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x76 /* HLT        */, () => { /* Placeholder? */ } },
            { 0x77 /* MOV M, A   */, () => { Array.Copy(State.Registers, 0x00, State.Memory, State.HLRegisterToAddress(), 0x01); } },
            { 0x78 /* MOV A, B   */, () => { Array.Copy(State.Registers, 0x01, State.Registers, 0x00, 0x01); } },
            { 0x79 /* MOV A, C   */, () => { Array.Copy(State.Registers, 0x02, State.Registers, 0x00, 0x01); } },
            { 0x7A /* MOV A, D   */, () => { Array.Copy(State.Registers, 0x03, State.Registers, 0x00, 0x01); } },
            { 0x7B /* MOV A, E   */, () => { Array.Copy(State.Registers, 0x04, State.Registers, 0x00, 0x01); } },
            { 0x7C /* MOV A, H   */, () => { Array.Copy(State.Registers, 0x05, State.Registers, 0x00, 0x01); } },
            { 0x7D /* MOV A, L   */, () => { Array.Copy(State.Registers, 0x06, State.Registers, 0x00, 0x01);} },
            { 0x7E /* MOV A, M   */, () => { Array.Copy(State.Memory, State.HLRegisterToAddress(), State.Registers, 0x00, 0x01); } },
            { 0x7F /* MOV A, A   */, () => { /* Essentially a NOP. */ } },
        };
